...
durable provenance fields. The complete Temporal workflow pages targets into concurrency-bounded child
workflows, executes restart-safe crawl, fingerprint, classification, selection, browser, analysis,
profile-persistence, embedding, and aggregation activities, and preserves partial success. The browser worker keeps a
...
Dataset creation selects eligible abstract scan records by ID and evaluates authorization, provenance, licensing, removal state, schema version, and quality policy. The control-plane API persists an idempotent `dataset_builds` attempt before dispatch. `DatasetBuildWorkflow` then runs bounded, restart-safe PostgreSQL activities that filter and deduplicate candidates, reject copied source content, calculate distributions, assign entire case-insensitive source domains to deterministic splits, write a quality report, and create the canonical manifest. A version is sealed only in the final transaction after every required check passes. Optional missing-embedding work is queued separately and is not a sealing prerequisite. Dataset bodies live in object storage; PostgreSQL holds lifecycle state, ownership, policy, lineage, checksums, and object keys. Optional embeddings live in Qdrant with the same dataset and provenance identifiers.
